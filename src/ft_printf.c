...
int	ft_print_types(t_dose *dose, va_list arg, int count)
{
	...
	else if (dose->format_conversion == 'd' ||
		dose->format_conversion == 'i' || dose->format_conversion == 'D')
...
int	ft_argument(const char **format_point, int count, va_list arg)
{
	...
	*format_point = adding_t_dose(format_point, dose);
	if (dose->format_conversion == 'd' || dose->format_conversion == 'i' ||
		dose->format_conversion == 'o' || dose->format_conversion == 'u' ||
		dose->format_conversion == 'x' || dose->format_conversion == 'X')
	{
		if (dose->period == 1)
			dose->zero = 0;
	}
	...
	if (dose->length_modifier[0] != '\0')
		ft_memdel((void**)&(dose->length_modifier));
	ft_memdel((void**)&dose);
	return (count);
}
